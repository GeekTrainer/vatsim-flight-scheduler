...
describe('vatsim', () => {
	...
	describe('fetchVatsimData - network failure scenarios', () => {
		beforeEach(() => {
			// Reset module state to clear cache between tests
			vi.resetModules();
		});

		it('should handle network timeout', async () => {
			const { fetchVatsimData } = await import('./vatsim');
			
			(global.fetch as any).mockRejectedValueOnce(new Error('Network timeout'));

			await expect(fetchVatsimData()).rejects.toThrow('Network timeout');
		});
		...
		it('should handle malformed JSON response', async () => {
			...
			await expect(fetchVatsimData()).rejects.toThrow('Unexpected token');
		});

		it('should handle empty controllers array', async () => {
			const { fetchVatsimData, getLocationControllers } = await import('./vatsim');
			
			const emptyData: VatsimData = {
				...mockVatsimData,
				controllers: []
			};
			...
			const locationControllers = getLocationControllers(data.controllers);
			expect(locationControllers.size).toBe(0);
		});



		it('should handle invalid logon_time format', async () => {
			const { fetchVatsimData, getLocationControllers } = await import('./vatsim');
			
			const invalidTimeData: VatsimData = {
				...mockVatsimData,
				controllers: [
					{
						cid: 1234567,
						...
						visual_range: 100,
						last_updated: '2026-01-04T12:00:00Z',
						logon_time: 'invalid-date-format'
					}
				]
			};
			...
			// Should handle invalid date gracefully - onlineTimeMinutes will be NaN or 0
			expect(twrControllers).toBeDefined();
			expect(twrControllers?.[0].callsign).toBe('ABQ_TWR');
		});
