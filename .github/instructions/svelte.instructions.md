...
### CLI Tools
- Use `npx sv create` to scaffold new SvelteKit projects with templates
- Use `npx sv add` to integrate features (testing, styling, auth, adapters, etc.)
- Use `npx sv check` for TypeScript/error checking and unused CSS detection
- Use `npx sv migrate` to upgrade Svelte/SvelteKit versions and refactor to runes
- Leverage `--from-playground` option to create projects from Svelte playground URLs
...
- Use SvelteKit stores for global application state when needed (legacy approach)
- Prefer `$app/state` module for accessing page state, navigation, and URL parameters
...
- Use snapshots to preserve form state across navigation
...
- Use remote functions for type-safe client-server communication without API endpoints
- Implement shallow routing for modals/dialogs that preserve navigation history
- Use link options (`data-sveltekit-preload-data`, `data-sveltekit-noscroll`) for performance
...
- Use `data-sveltekit-preload-data` on links for eager data fetching
- Implement observability with OpenTelemetry for production monitoring
- Use shallow routing to avoid full page reloads for overlays/modals

### Data Fetching
- Use SvelteKit's load functions for server-side and universal data fetching
- Implement remote functions for type-safe server calls without creating API routes
- Use proper loading, error, and success states
...
- Use link preloading options to eagerly fetch data before navigation
...
- Remote functions for CRUD operations without creating API routes
- Shallow routing for modals, image galleries, and overlays
- Form state snapshots to preserve user input across navigation
- Link preload strategies for instant navigation experiences

## Remote Functions Pattern
Remote functions enable type-safe server communication without API endpoints:
...
import { getUser } from './+page.server';

const user = await getUser('123'); // Type-safe, no fetch needed
```

## Shallow Routing Pattern
Use shallow routing for overlays that shouldn't trigger full page loads:
